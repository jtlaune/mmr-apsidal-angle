...
#+BEGIN_SRC jupyter-python :session /jpy:localhost#8888:research
  ...
  # Varying parameters #
  ######################
  eccs = np.linspace(0.02, 0.08, int(np.sqrt(Nqs)))
  ...
  NAMES = np.array([f"e1d-{E1_0[i]:0.3f}-e2d-{E2_0[i]:0.3f}"
                     for i, qit in enumerate(QS)])
  
  ################
  # WITH SECULAR #
  ################
  DIRNAMES = np.array([f"./hatR-phasegrid-q{int(QS[i])}-TE1{TE1[0]}-TE2{TE2[0]}" for i
                          in range(Nqs)])
  ...
  for i, filename in enumerate(NAMES):
      ...
      aligned_arr[i,4] = align
      aligned_arr[i,5] = antialign
  np.savetxt(os.path.join(".", f"behaviors-Rhat-q{q}-TE1{TE1[0]}-TE2{TE2[0]}.txt"), aligned_arr)
#+END_SRC
...
#+BEGIN_SRC jupyter-python :session /jpy:localhost#8888:research
  ...
  dirname = f"."
  params = np.loadtxt(os.path.join(".", f"behaviors-Rhat-q{q}-TE1{TE1[0]}-TE2{TE2[0]}.txt"))
  ...
  ax.set_xlabel(r"$e_{1,\rm d}$",fontsize=fontsize)
  ax.set_ylabel(r"$e_{2,\rm d}$",fontsize=fontsize)
  ax.tick_params(axis="both", labelsize=fontsize)
  ...
  fig.savefig(os.path.join(dirname, f"align-Rhat-q{q}-TE1{TE1[0]}-TE2{TE2[0]}.png"), bbox_inches="tight")
#+END_SRC

#+RESULTS:
: <ipython-input-121-e247b60919ee>:20: MatplotlibDeprecationWarning: shading='flat' when X and Y have the same dimensions as C is deprecated since 3.3.  Either specify the corners of the quadrilaterals with X and Y, or pass shading='auto', 'nearest' or 'gouraud', or set rcParams['pcolor.shading'].  This will become an error two minor releases later.
:   cs = ax.pcolormesh(e1s, e2s, avg_hatRs_grid)
...
#+BEGIN_SRC jupyter-python :session /jpy:localhost#8888:research
  ...
  def Q2(j, q, sgn):
      ...
      return(sgn*prefac*(q*sqrt(alpha0)+j*(q/alpha0+1)))
  ...
  Te1 = 1000
  Tm = 1e5
  E1S_SQ_branch1 = Te1/2/Q1(j, QS, 1)/Tm/(1+TAUS*Q2(j,QS,-1)/Q1(j,QS,1))
  E1S_SQ_branch2 = Te1/2/Q1(j, QS, -1)/Tm/(1+TAUS*Q2(j,QS,1)/Q1(j,QS,-1))
  
  
  E1S_branch1 = sqrt(E1S_SQ_branch1)
  E1S_branch2 = sqrt(E1S_SQ_branch2)
  
  E2S_branch1 = sqrt(alpha0)*QS*np.abs(f2/f1)*E1S_branch1
  E2S_branch2 = sqrt(alpha0)*QS*np.abs(f2/f1)*E1S_branch2
  
  fig, ax = plt.subplots(1,2,figsize=(10,5))
  
  cs1 = ax[0].pcolormesh(TAUS,QS,E1S_branch2, norm=mpl.colors.LogNorm())
...
#+RESULTS:
:RESULTS:
: /tmp/ipykernel_20631/3986406627.py:25: RuntimeWarning: invalid value encountered in sqrt
:   E1S_branch1 = sqrt(E1S_SQ_branch1)*1
: /tmp/ipykernel_20631/3986406627.py:26: RuntimeWarning: invalid value encountered in sqrt
:   E1S_branch2 = sqrt(E1S_SQ_branch2)*1
: /tmp/ipykernel_20631/3986406627.py:33: MatplotlibDeprecationWarning: shading='flat' when X and Y have the same dimensions as C is deprecated since 3.3.  Either specify the corners of the quadrilaterals with X and Y, or pass shading='auto', 'nearest' or 'gouraud', or set rcParams['pcolor.shading'].  This will become an error two minor releases later.
:   cs1 = ax[0].pcolormesh(TAUS,QS,E1S_branch2, norm=mpl.colors.LogNorm())
| 0.5 | 2.0 |
[[file:./.ob-jupyter/2598140a21c09e3a90a12fcfe3151d8acf4eb001.png]]
:END:

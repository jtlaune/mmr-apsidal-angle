...
#+RESULTS:
:RESULTS:
: /tmp/ipykernel_30952/1765676941.py:9: UserWarning: No contour levels were found within the data range.
:   cs1 = ax.contour(Q, H, hcond, levels=0., inline=True)
: /tmp/ipykernel_30952/1765676941.py:9: UserWarning: The following kwargs were not used by contour: 'inline'
:   cs1 = ax.contour(Q, H, hcond, levels=0., inline=True)
# [goto error]
: ---------------------------------------------------------------------------
: NameError                                 Traceback (most recent call last)
: /tmp/ipykernel_30952/1765676941.py in <module>
:       8     hcond = np.abs(np.abs(e1d-H*sqrt(Q))-H) < Delta
:       9     cs1 = ax.contour(Q, H, hcond, levels=0., inline=True)
: ---> 10     labelAtEdge(cs1.levels, cs1, ax, f"{e1d:0.2f}", side='right', pad=0.2)
:      11 ax.set_ylabel("h")
:      12 ax.set_xlabel("q")
: 
: NameError: name 'labelAtEdge' is not defined
[[file:./.ob-jupyter/68c1454c7345795e36c7fa0d2aa3cd7d43148629.png]]
:END:
...
#+BEGIN_SRC jupyter-python :session /jpy:localhost#8888:research
  def plotanal(plot_iter, cut_iter=[0,-1]):
      ...
      hattheta1s = np.arctan2(e1*sin(theta1) + f2/f1*e2*sin(theta2),
                            e1*cos(theta1) + f2/f1*e2*cos(theta2))
      hattheta1s = hattheta1s %(2*np.pi)
      hattheta1_avg = np.arctan2(np.sum(np.sin(hattheta1s)), np.sum(np.cos(hattheta1s)))
      hattheta1_std = np.sqrt(np.sum(hattheta1s-hattheta1_avg)**2)/len(hattheta1s)
  
      Rhat = np.abs(f1**2*e1**2+f2**2*e2**2+f1*f2*e1*e2*cos(Dpoms))
      ehat = np.sqrt(Rhat)
      fig, ax = plt.subplots(3, figsize=(10,10)) 
      ax[0].scatter(teval, e1, s=0.5, label="e1", c="k")
      ax[0].scatter(teval, e2, s=0.5, label="e2", c="r")
      ax[0].scatter(teval, ehat, s=0.5, label="Rhat",c="g")
  
      C0 = mpl.lines.Line2D([], [], color='k', marker="o", linestyle='None',
                            markersize=5, label=r'$e_1$')
      C1 = mpl.lines.Line2D([], [], color='r', marker="o", linestyle='None',
                            markersize=5, label=r'$e_2$')
      C2 = mpl.lines.Line2D([], [], color='g', marker="o", linestyle='None',
                            markersize=5, label=r'$\hat e$')
  
      ax[0].legend(handles=[C0, C1, C2], loc="lower right", ncol=1, framealpha=1)
  
  
      ax[1].scatter(teval, Dpoms*180/pi, s=0.5, alpha=0.2, c="k")
      ax[2].scatter(teval, theta1*180/pi, s=0.5, alpha=0.2, c="k")
      ax[2].scatter(teval, theta2*180/pi, s=0.5, alpha=0.2, c="r")
      ax[2].scatter(teval, hattheta1s*180/pi, s=0.5, alpha=0.2, c="g")
  
      C0 = mpl.lines.Line2D([], [], color='k', marker="o", linestyle='None',
                            markersize=5, label=r'$\theta_1$')
      C1 = mpl.lines.Line2D([], [], color='r', marker="o", linestyle='None',
                            markersize=5, label=r'$\theta_2$')
      C2 = mpl.lines.Line2D([], [], color='g', marker="o", linestyle='None',
                            markersize=5, label=r'$\hat \theta$')
  
      ax[2].legend(handles=[C0, C1, C2], loc="upper right", ncol=1, framealpha=1)
  
      for axi in ax.flatten():
          axi.tick_params(which="major", width=3, length=8,
                          bottom=True, top=True, left=True, right=True,
                          direction="in", pad=10)
          axi.tick_params(which="minor", width=3, length=4,
                          bottom=True, top=True, left=True, right=True,
                          direction="in", pad=10)
          axi.set_xlim((teval[0], teval[-1]))
      ...
      ax[2].set_ylim((0,360))
      ax[0].set_ylabel(r"$e$")
      ax[1].set_ylabel(r"$\Delta\varpi$")
      ax[2].set_ylabel(r"$\theta$")
      ax[2].set_xlabel(r"$t [yrs]$")
...
#+BEGIN_SRC jupyter-python :session /jpy:localhost#8888:research
  leng = 300000
  plotanal(1,cut_iter=[int(0.1*leng),int(0.11*leng)])
#+END_SRC
...
| <Figure | size | 720x720 | with | 3 | Axes> | array | ((<AxesSubplot:ylabel= $e$ > <AxesSubplot:ylabel= $\Delta\varpi$ > <AxesSubplot:xlabel= $t [yrs]$ ylabel= $\theta$ >) dtype=object) |
[[file:./.ob-jupyter/8137ae45bd0e192f520c218d4b6973b856bd8266.png]]
:END:

#+BEGIN_SRC jupyter-python :session /jpy:localhost#8888:research
  leng = 300000
  fig, ax = plotanal(1,cut_iter=[int(0.8*leng),int(0.9*leng)])
  
  os.chdir("/home/jtlaune/multi-planet-architecture/notes/")
  fig.savefig("resmode.png")
#+END_SRC
...
: 16
: ./driveTe-h-0.03-mutot-1.0e-04-Tw0-1000-q2.0/e1d-0.1-e2d-0.0.npz
[[file:./.ob-jupyter/aff72d3bc0a27762a62a91f24997cad8e78176a3.png]]
:END:
...
#+BEGIN_SRC jupyter-python :session /jpy:localhost#8888:research
  ...
  f2 = 2.5
  Dpom = np.pi
  qs = np.logspace(np.log10(0.5), np.log10(2), 1000)
  R1 = e1d**2*np.ones(len(qs))
  R2 = R1*1.42*qs**(4./3)
  Rhat = R1*(1/(qs+1))**(2./3)*(f1**2+f2**2*qs**2-2*np.abs(f1*f2)*qs*cos(Dpom))
  fig, ax = plt.subplots(1,2,figsize=(10,5))
  ax[0].plot(-sqrt(R1), qs, c="k")
  ax[0].plot(sqrt(R2), qs, c="r")
  ax[0].plot(-sqrt(Rhat), qs, c="g")
  ax[0].plot(-sqrt(R1)-sqrt(R2), qs, c="g", ls="--")
  ax[0].axvline(x=0, c="k", ls="--")
  ax[1].plot(qs, Rhat - (sqrt(R1)-sqrt(R2)))
#+END_SRC

#+RESULTS:
:RESULTS:
| <matplotlib.lines.Line2D | at | 0x7f97e33a04f0> |
[[file:./.ob-jupyter/42d6f2c5744773ce78de2459147a877ba353643f.png]]
:END:
...
#+BEGIN_SRC jupyter-python :session /jpy:localhost#8888:research
  ...
  R1 = e1d**2*np.ones(len(qs))
  R2 = R1*1.42
  Rhat = R1*(1/(qs+1))**(2./3)*(f1**2+f2**2-2*np.abs(f1*f2)*cos(Dpom))
  fig, ax = plt.subplots(1,2,figsize=(10,5))
  ax[0].plot(-sqrt(R1), qs, c="k")
  ax[0].plot(sqrt(R2), qs, c="r")
  ax[0].plot(-sqrt(Rhat), qs, c="g")
...
#+RESULTS:
:RESULTS:
| <matplotlib.lines.Line2D | at | 0x7f97e320f520> |
[[file:./.ob-jupyter/ab2ca85434ca564c66adba38d65df4f6332361d6.png]]
:END:

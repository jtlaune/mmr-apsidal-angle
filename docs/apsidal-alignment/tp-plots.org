...
#+BEGIN_SRC jupyter-python :session /jpy:localhost#8888:research
  ...
  for j, h in enumerate(hs):
      ...
      # for ext particle
      xi = 1. # h = xi*sqrt(j/1.73)*e fudge factor
      j = 2. 
      alpha2 = ((j+1)/j)**(2./3)
      cosDpom = 1
      f1 = A(1/alpha2, j)
...
:RESULTS:
#+begin_example
  /tmp/ipykernel_4378/1320158720.py:58: ComplexWarning: Casting complex values to real discards the imaginary part
    eeqs[i] = rts_R[rts_win][0]
  /tmp/ipykernel_4378/1320158720.py:58: ComplexWarning: Casting complex values to real discards the imaginary part
    eeqs[i] = rts_R[rts_win][0]
  no root for ep = 0.15
  ...
  no root for ep = 0.15
#+end_example
[[file:./.ob-jupyter/6710950f6c302afbd66a8707b5c0e6a37871822e.png]]
:END:

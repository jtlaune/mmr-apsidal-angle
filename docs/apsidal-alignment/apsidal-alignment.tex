...
used in the literature.  In Section \ref{sec:orgd1676c5}, we present the classical picture of resonant capture and
...
conditions which robustly lead to apsidal alignment.  In Section \ref{sec:orgc158d08},
...
\ref{sec:org7fe9cf8}, where we design a toy model which relies on
...
Section \ref{sec:orgd1676c5} for the test particle.
...
\(q\to\infty\)), see Section \ref{sec:org416a35c}. In the same section,
...
found in Section \ref{sec:orgd1676c5}.
...
outer planet.  As we saw earlier in Section \ref{sec:orgc158d08}, the
...
Section \ref{sec:orgc158d08}, with the goal of a heuristic
understanding of apsidal alignment in real planetary systems (i.e. \(0<q<\infty\)).
...
which we numerically investigated in Section \ref{sec:org7fe9cf8} (i.e. the
...
quantities, we use \(e_{\rm eq}\) \(= h\sqrt{1.73/j}\) (equation
\eqref{eq:eeqext}) as a proxy for the ``disk-driven eccentricity'' of the test particle.  One can see in Figure \ref{fig:Rhat-grid} that, for
...
equivalent dynamical states.  Similarly, the apsidally-aligned regions
of the \(q=\infty\) and \(q=2\) parameter spaces also correspond with each
other.

\subsubsection{Calculation of the phase space interface}
\label{sec:org5219177}
We again adopt the notation used in Section \ref{sec:orgc158d08} for \(q=\infty\).
...
\(\theta\) librate (Figure \ref{fig:tp-align}).  The angle \(\hat\theta\)
(as written in equation \eqref{eq:tphhat}) librates around zero in both
the aligned and circulating cases.  Hence, the upper left triangular
regions (apsidal circulation) of phase space for \(q=\infty\) has both
\(\theta\) and \(\hat\theta\) (\(\theta_2\) and \(\hat\theta\) for \(q=2\))
librating, while the lower right triangular regions (apsidal
alignment) have only \(\hat\theta\) librating.

The angle \(\theta\) therefore determines the behavior of
\(\Delta\varpi\) in resonance for a test particle.  Based on the results
depicted in Figure \ref{fig:tp-grid-ext}, we assume that the division
between apsidal circulation and alignment is linear at these
eccentricity scales. We must therefore find a slope, \(dh/de_p\), and an
\(e_p\) intercept.

Consider the denominator of equation \eqref{eq:tphhat}.  For small
\(e_p\), in the apsidal circulation region, \(\theta\) librates near
\(180^\circ\) and so \(e\cos\theta\approx -e\).  The denominator then
becomes \(\approx -e -|f_1/f_2|e_p\cos\theta_p\).  Because \(\theta_p\)
always circulates, the second term oscillates between \(-|f_1/f_2|e_p\)
and \(|f_1/f_2|e_p\).  Hence, in order for \(\theta\) and \(\hat\theta\) to
both librate, the denominator must obey
\begin{align}
  \label{eq:tphhat}
  -e + \left|\frac{f_1}{f_2}\right|e_p > 0
\end{align}

\noindent
which implies the line dividing phase space in Figure
\ref{fig:tp-grid-ext} should have have slope
\begin{align}
  \frac{de_{\rm eq}}{de_p} \sim \left|\frac{f_1}{f_2}\right|.
\end{align}

\noindent As one can see in Figures \ref{fig:tp-ep0}, \ref{fig:tp-circ} and
\ref{fig:tp-align}, \(e\) oscillates between roughly \(0\) and
\(e_{\rm eq} + \mathcal O(e_p)\). The latter term we estimate with
\begin{align}
  e_{p,0} \equiv e_{\rm eq}(h_{\rm min}) = h_{\rm min}\sqrt{\frac{1.73}{j}},
\end{align}

\noindent where \(h_{\rm min}=0.015\) is the smallest aspect ratio in
Figure \ref{fig:tp-grid-ext} (see Table \ref{tab:ICs}).
After converting back to \(h\), again utilizing the formula for \(e_{\rm eq}\),
we estimate the transition to aligned perihelia to occur for \(h>h_{c}(e_p)\), where \(h_c\)
is the function given by
\begin{align}
\label{eq:phasespaceline}
  h_c(e_p) = \sqrt{\frac{j}{1.73}}\left|\frac{f_1}{f_2}\right|e_p - 0.015.
\end{align}

\noindent We plot this equation in Figure \ref{fig:tp-grid-ext}.
Despite our many approximations, this analysis reproduces the
numerical results fairly well.

\subsubsection{Discussion for \(0<q<\infty\)}
\label{sec:org6a965d3}
In the previous sections, we have demonstrated that test particle
apsidal alignment can be understood as a competition between the
magnitude of \(h\), which characterizes the relative strength of disk
damping forces and sets \(e_{\rm eq}\), and \(e_p\), the eccentricity of
the perturber.  The natural interpretation of these results in the
comparable mass context is to identify the disk driven eccentricities
of Section \ref{sec:org7fe9cf8} with \(e_{\rm eq}\) and \(e_p\) in Section \ref{sec:orgc158d08}.

Apsidal behavior in the comparable mass regime is more complex than
the ER3BP.  We observe \(|\Delta\varpi|=0\), \(=90^\circ\), and
\(=180^\circ\).  Each planet's eccentricity works
on the other to anti-align their periapses in
resonance. However, if disk forces drive them \emph{away} from the ratio
\(e_1/e_2=1/q\), the anti-alignment can be overcome, resulting in a
different configuration.  The resulting configuration has a
complicated dependence on the planet and disk parameters, with some
systems escaping resonance (see Figure \ref{fig:Rhat-grid}).
It is likely that a detailed analysis of the parameters \(q\), \(h\),
\(T_e\), and \(e_{i,d}\) would reveal a set of 3 equations analagous to
equation \eqref{eq:phasespaceline} for the comparable mass regime,
but that is beyond the scope of this paper.

\section{Conclusion}
\label{sec:org16190aa}
...
Our investigation began by analyzing the problem of a test particle
in the vicinity of an MMR with a planet of mass \(m_p\) (Section \ref{sec:orgc158d08}).  Here, we briefly reviewed the textbook treatment
...
mass case in Section \ref{sec:org7fe9cf8}.  While previous studies typically
...
\label{sec:orga918310}
\subsection{Scaling the Hamiltonian}
\label{sec:org258f2db}
...
in Appendix \ref{sec:orga918310}, which is generated by the infinitessimal rotation

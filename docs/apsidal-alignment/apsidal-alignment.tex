...
used in the literature.  In Section \ref{sec:org8effac2}, we present the classical picture of resonant capture and
...
conditions which robustly lead to apsidal alignment.  In Section \ref{sec:org7f87d1c}, we explore apsidal alignment for a test particle in
the vicinity of an eccentric planet's MMR. These results guide our
analysis in Section \ref{sec:orgdc2ff3f}, where we design a toy model which
...
Section \ref{sec:org8effac2} for the test particle.
...
\(q\to\infty\)), see Section \ref{sec:orgd1729fb}. In the same section,
...
the details of these transformations, see Appendix \ref{sec:org3718782}.
...
found in Section \ref{sec:org8effac2}.
...
outer planet.  As we saw earlier in Section \ref{sec:org7f87d1c}, the
...
The results of Section \ref{sec:org0229819} can be applied to the
...
Section \ref{sec:org7f87d1c}.  As a first order approximation, we
...
in Section \ref{sec:org8effac2}.  However, there are
...
in the vicinity of an MMR with a planet of mass \(m_p\) (Section \ref{sec:org7f87d1c}).  Here, we briefly reviewed the textbook treatment
...
mass case in Section \ref{sec:orgdc2ff3f}.  While previous studies typically
...
\label{sec:org3718782}
\subsection{Scaling the Hamiltonian}
\label{sec:orgda00e92}

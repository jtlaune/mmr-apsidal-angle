...
used in the literature.  In Section \ref{sec:org68e4a61}, we present the classical picture of resonant capture and
...
conditions which robustly lead to apsidal alignment.  In Section \ref{sec:org1489500}, we explore apsidal alignment for a test particle in
the vicinity of an eccentric planet's MMR. These results guide our
analysis in Section \ref{sec:org2e94755}, where we design a toy model which
...
Section \ref{sec:org68e4a61} for the test particle.
...
\(q\to\infty\)), see Section \ref{sec:org9fca5d4}. In the same section,
...
the details of these transformations, see Appendix \ref{sec:org9ff964e}.
...
found in Section \ref{sec:org68e4a61}.
...
The results of Section \ref{sec:org359e567} can be applied to the
...
Section \ref{sec:org1489500}.  As a first order approximation, we
...
\(\Delta\varpi=\theta_1-\theta_2=180^\circ\) in resonance.  However,
there are observed exoplanets near resonance which have
\(\Delta\varpi\approx0^\circ\), such as K2-19.
In this paper, we investigated if resonant dynamics could align
\(\varpi_1\) and \(\varpi_2\).

This investigation began by analyzing the problem of a test particle
in the vicinity of an MMR with a planet of mass \(m_p\) (Section \ref{sec:org1489500}).
Here, we briefly reviewed
...
induced by the disk's damping forces.
...
system finds an equilibrium where \(e\) librates around \(e_p\).

This lead us to to study the effects of an
eccentricity driving force on the dynamics of two comparable mass
planets near a \(j:j+1\) MMR.  We observed apsidal alignment for strong
enough damping forces away from the line \(e_2/e_1 \approx q\).
...
apsidal alignment if the system remains captured into resonance.
  \item Eccentricity driving forces can prevent the libration of $\theta_1$ and/or $\theta_2$.
\end{enumerate}
...
\label{sec:org9ff964e}
\subsection{Scaling the Hamiltonian}
\label{sec:org4432559}

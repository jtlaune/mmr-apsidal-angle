...
the details of these transformations, see Appendix \ref{sec:orge5b2fe8}.  The parameter \(\delta\)
...
Define \(\v{\hat e} = \abs{f_1}\v e_1 -
\abs{f_2}\v e_2\), where \(\v e_i\) is the Runge-Lenz vector, i.e. the
vector with magnitude \(e_i\) in the direction of perihelion.  In the
outer test particle limit where \(e_1\) and \(m_2\) approach 0 and \(q\)
approaches infinity, the momentum \(\hat R\) is proportional to
\begin{align}
\label{eq:R1limit}
  \hat R \propto \frac{\hat e^2}{q^{2/3}\hat\mu^{2/3}}
  ;\quad R_2 \equiv \lim_{m_2 \to 0} \hat R
  \propto  \frac{e_2^2}{\mu_1^{2/3}} .
\end{align}

\noindent The quantity \(R_1\) is the standard action for the
circular restricted three-body problem (CR3BP) and is conjugate to the
angle \(\theta_1\).  In the opposite limit for an inner test particle,
the action becomes
\begin{align}
\label{eq:R2limit}
  \hat R \propto \frac{q^{2/3}\hat e^2}{\hat\mu^{2/3}}
  ;\quad R_1 \equiv \lim_{m_1\to 0} \hat R
  \propto \frac{e_1^2}{\mu_2^{2/3}}.
\end{align}
...
\label{sec:orge5b2fe8}
\subsection{Scaling the Hamiltonian}
\label{sec:orgc2f617b}

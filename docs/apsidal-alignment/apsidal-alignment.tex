...
used in the literature.  In Section \ref{sec:orgd86b59e}, we present the classical picture of resonant capture and
...
conditions which robustly lead to apsidal alignment.  In Section \ref{sec:orgb7de15e},
...
\ref{sec:orgc8aac6d}, where we design a toy model which relies on
...
Section \ref{sec:orgd86b59e} for the test particle.
...
\(q\to\infty\)), see Section \hyperref[sec:orgb690b0a]{Relation to the test particle limit}. In the
...
found in Section \ref{sec:orgd86b59e}.
...
outer planet.  As we saw earlier in Section \ref{sec:orgb7de15e}, the
...
\label{sec:orgb690b0a}
In Section \ref{sec:orgc8aac6d}, we shifted
our study of apsidal alignment from the test particle treatment
(Section \ref{sec:orgb7de15e}) to comparable mass planets. We
introduced the reducing rotation, which transforms the Hamiltonian in
equation \eqref{hamiltonian} into the single dof form in equation
\eqref{hhat}. We then numerically investigated the apsidal
behavior of systems in resonance for \(1/2\geq q\leq 2\).

The perturbation Hamiltonian in equation \eqref{eq:Hpert1} is linear
with coupling coefficient \(q\), and so the comparable mass case can be
thought of as a superposition of this mechanism.  Relative to the
comparable mass case, the test particle analysis simplifies the
problem considerably because the vector \(\mathbf{e}_p\) remains
constant in time.  Because of this, we now return to the test particle
treatment, with the goal of a heuristic understanding of apsidal
alignment in planetary systems.

We again adopt the notation used in
Section \hyperref[sec:orgb7de15e]{Test Particle Results}. The inner planet has mass \(m_p\)
(subscript \(p\)) and the outer planet is a test particle (no
subscript).
...
in Section \ref{sec:orgd86b59e}.  However, there are
...
in the vicinity of an MMR with a planet of mass \(m_p\) (Section \ref{sec:orgb7de15e}).  Here, we briefly reviewed the textbook treatment
...
mass case in Section \ref{sec:orgc8aac6d}.  While previous studies typically
...
\label{sec:org32ecc00}
\subsection{Scaling the Hamiltonian}
\label{sec:orgef58056}
...
in Appendix \ref{sec:org32ecc00}, which is generated by the infinitessimal rotation

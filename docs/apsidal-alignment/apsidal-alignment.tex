...
used in the literature.  In Section \ref{sec:org941a59e}, we present the classical picture of resonant capture and
...
conditions which robustly lead to apsidal alignment.  In Section \ref{sec:orgb1013e8},
...
\ref{sec:orgc6da531}, where we design a toy model which relies on
...
Section \ref{sec:org941a59e} for the test particle.
...
\(q\to\infty\)), see Section \hyperref[sec:orgb887201]{Relation to the test particle limit}. In the
...
found in Section \ref{sec:org941a59e}.
...
outer planet.  As we saw earlier in Section \ref{sec:orgb1013e8}, the
...
Section \ref{sec:orgb1013e8}, with the goal of a heuristic
understanding of apsidal alignment in planetary systems.

For simplicity, we restrict ourselves to an external test particle
(Figure \ref{fig:tp-grid-ext}), which corresponds to the formal limit of
equation \eqref{hamiltonian} as \(q\to\infty\).
We then compare it to the \(q=2\) case, which we numerically investigated in
Section \ref{sec:orgc6da531} (i.e. the first two rows Figure \ref{fig:Rhat-grid}).
For consistency, we again adopt the notation used in Section \ref{sec:orgb1013e8}.  The inner planet has mass \(m_p\) (subscript \(p\)) and
...
In resonance (Figures \ref{fig:tp-ep0} - \ref{fig:tp-align})
...
in Section \ref{sec:org941a59e}.  However, there are
...
in the vicinity of an MMR with a planet of mass \(m_p\) (Section \ref{sec:orgb1013e8}).  Here, we briefly reviewed the textbook treatment
...
mass case in Section \ref{sec:orgc6da531}.  While previous studies typically
...
\label{sec:org46b1914}
\subsection{Scaling the Hamiltonian}
\label{sec:org9b95cab}
...
in Appendix \ref{sec:org46b1914}, which is generated by the infinitessimal rotation

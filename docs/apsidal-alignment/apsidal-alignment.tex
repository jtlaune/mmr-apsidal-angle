...
used in the literature.  In Section \ref{sec:org675967f}, we present the classical picture of resonant capture and
...
conditions which robustly lead to apsidal alignment.  In Section \ref{sec:orge78f4f3},
...
\ref{sec:orgea17017}, where we design a toy model which relies on
...
Section \ref{sec:org675967f} for the test particle.
...
\(q\to\infty\)), see Section \ref{sec:orgca95816}. In the same section,
...
found in Section \ref{sec:org675967f}.
...
outer planet.  As we saw earlier in Section \ref{sec:orge78f4f3}, the
...
Section \ref{sec:orge78f4f3}, with the goal of a heuristic
...
which we numerically investigated in Section \ref{sec:orgea17017} (i.e. the
...
\label{sec:org01119ea}
We again adopt the notation used in Section \ref{sec:orge78f4f3} for \(q=\infty\).
...
of Section \ref{sec:orgea17017} with \(e_{\rm eq}\) and \(e_p\) in Section \ref{sec:orge78f4f3}.
...
in Section \ref{sec:org675967f}.  However, there are
...
in the vicinity of an MMR with a planet of mass \(m_p\) (Section \ref{sec:orge78f4f3}).  Here, we briefly reviewed the textbook treatment
...
mass case in Section \ref{sec:orgea17017}.  While previous studies typically
...
\label{sec:orgee7ef8a}
\subsection{Scaling the Hamiltonian}
\label{sec:orgc016a7e}
...
in Appendix \ref{sec:orgee7ef8a}, which is generated by the infinitessimal rotation

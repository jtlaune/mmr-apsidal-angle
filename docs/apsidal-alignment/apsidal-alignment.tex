...
\citep{lin79_tidal_torques_accret_discs_binar,goldreich_excitation_1979,goldreich_disk-satellite_1980-1}.
...
\citep{nelson_planetary_2018}. More recent theoretical works have
examined the possibility of the PPD driving the planets eccentricity
to a finite saturation value rather than damping it to zero
...
In \citet{romero21_eccen_drivin_pebbl_accret_low_mass_planet}, the
authors found that luminous, pebble-accreting protoplanets can be
driven to eccentricities beyond the disk's aspect ratio.

When two planets' periods are related by an integer fraction \(j\):\(j+k\)
(e.g. 1:3, 2:3), mutual gravitational interactions can lock orbits
...
used in the literature.  In Section \ref{sec:orgdfdcb16}, we present the classical picture of resonant capture and
...
conditions which robustly lead to apsidal alignment.  In Section \ref{sec:org12dca5d}, we explore apsidal alignment for a test particle in
the vicinity of an eccentric planet's MMR. These results guide our
analysis in Section \ref{sec:orgbedf628}, where we design a toy model which
...
the primary. Details can be found in Appendix \ref{sec:org0d4779e} The quantity \(J\) is the angular momentum
...
\ref{fig:eqDpom}. In all cases, the analytic equilibrium equations
predict \(\Delta\varpi\approx 180^\circ\), and the numerical
...
the line \(T_{e,1}/T_{e_2} = 1\) (i.e. \(q=1\) in Figure
\ref{fig:standard}), which is where we reverse the migration direction
to ensure it is convergent. Variations in the eccentricity damping
ratio cannot account for apsidal alignment; \(e_1\) and \(e_2\) always
adjust to satisfy equation \eqref{dotdpom}.  In order to simplify the
problem, we turn to the case of a test particle orbiting near an MMR
with a finite mass planet. The planet's orbit is unchanging, and so we
may isolate the effect of eccentricity on the apsidal angle.

\section{Test Particle Results}
\label{sec:org12dca5d}
...
Section \ref{sec:orgdfdcb16} for the test particle.
...
\label{sec:org74b59e7}
The Hamiltonian, including secular effects, for a test particle in the
vicinity of an interior MMR is
(c.f. \citet{xu_migration_2018,goldreich_overstable_2014,wisdom_canonical_1986,deck_migration_2015,henrard86_reduc_trans_apocen_librat})
\begin{align*}
  \label{eq:tpint}
  H
...
\(q\to\infty\)), see Section \ref{sec:orgf336730}.

\section{Apsidal Angle}
\label{sec:orgbedf628}
...
the details of these transformations, see Appendix \ref{sec:org0d4779e}.  The parameter \(\delta\)
...
found in Section \ref{sec:orgdfdcb16}.
...
\label{sec:orgf336730}
The results of Section \ref{sec:org0cc8cdd} can be applied to the
...
\(m_2\) ensure \(e_1\) librates in amplitude.
...
\(\Delta\varpi=\theta_1-\theta_2=180^\circ\) in resonance.  Next, we
briefly reviewed the textbook treatment of the CR3BP and its
extension, the ER3BP, and found that apsidal alignment occurs for
large enough values of \(e_p\). These results prompted the addition of
an eccentricity driving force and informed our analytical analysis of
how apsidal alignment may arise in the case of \(q\sim\mathcal{O}(1)\).
...
paper: (1) an external eccentricity driving force can produce apsidal
alignment and (2) it can also destroy the resonance.  The apsidal
alignment in K2-19b and c, along with their moderate eccentricities
and the fact that we observe \(\hat\theta\) to librate
\citep{petit_resonance_2020}, therefore suggests that the planets are
interacting with an external force.  However, our second conclusion
implies that such a force could potentially destabilize and destroy
the resonance in the future.
...
still be in resonance if they are found to be eccentric.  We also
found evidence that some systems remain locked near \(P_2/P_1=1.5\) for
long timescales after exiting resonance due to \(e_{1d}\) and \(e_{2d}\).
\twocolumn
...
\label{sec:org0d4779e}
\subsection{Scaling the Hamiltonian}
\label{sec:org2b5ecec}

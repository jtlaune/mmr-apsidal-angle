...
used in the literature.  In Section \ref{sec:org2d9f5c1}, we present the classical picture of resonant capture and
...
conditions which robustly lead to apsidal alignment.  In Section \ref{sec:orga3cd90f},
...
\ref{sec:org16632a3}, where we design a toy model which relies on
...
Section \ref{sec:org2d9f5c1} for the test particle.
...
\(q\to\infty\)), see Section \hyperref[sec:orgd119949]{Relation to the test particle limit}. In the
same section, we will also describe how we calculate the dashed line
in Figure \ref{fig:tp-grid-ext}, which depicts our analytical estimate
for the boundary between the $\varpi$-aligned and
$\varpi$-circulating regions.

\section{Apsidal Angle}
\label{sec:org16632a3}
...
found in Section \ref{sec:org2d9f5c1}.
...
outer planet.  As we saw earlier in Section \ref{sec:orga3cd90f}, the
...
\subsection{Relation to the test particle limit}
\label{sec:orgd119949}
The test particle Hamiltonian in \eqref{eq:tpint} can also
be transformed into the form of equation \eqref{hhat}.
...
This implies that the limit of the comparable mass Hamiltonian (equation \eqref{hamiltonian})
must reduce to the restricted case \citep{deck13_first_order_reson_overl_stabil}.
We may therefore return to the test particle treatment in Section
\ref{sec:orga3cd90f} in order to understand the apsidal alignment
process in Section \ref{sec:org16632a3}.
In particular, we restrict ourselves to an external test particle,
which corresponds to the limit as \(q\) approaches infinity.
We compare it to the \(q=2>1\) case which we numerically investigated
in Section \ref{sec:org16632a3} (Figure \ref{fig:Rhat-grid}).

we the apsidal behavior of systems in resonance for \(1/2\geq
q\leq 2\).

Because the resonant Hamiltonian in equation
\eqref{eq:Hcompmassres} is a superposition
of terms involving \(e_1\) and \(e_2\), the

of terms corresponding to 

linear with coupling coefficient \(q\), the
comparable mass case is \emph{roughly} the superposition of two 

dynamical mechanisms involved in the alignment process of two independent .

Relative to the comparable mass case, the test particle
analysis simplifies the problem considerably because the vector
\(\mathbf{e}_p\) remains constant in time.  Because of this, we now
return to the test particle treatment, with the goal of a heuristic
understanding of apsidal alignment in planetary systems.

For consistency, we again adopt the notation used in Section \ref{sec:orga3cd90f}.  The inner planet has mass \(m_p\) (subscript \(p\)) and
the outer planet is a test particle (no subscript). This setup
formally corresponds to the limit of \(q\) approaching infinity in the
comparable mass Hamiltonian.
...
which we denote by \(e_{\rm eq}\), and the parameter \(e_p\).  

We denote the angles which correspond to \(\theta_1\) and \(\theta_2\) as
\(\theta_p=(j+1)\lambda-\lambda_p\) and \(\theta= \theta_p-\varpi\).  The
resonant angle \(\hat\theta\) can now be written as
\begin{align}
...
in Section \ref{sec:org2d9f5c1}.  However, there are
...
in the vicinity of an MMR with a planet of mass \(m_p\) (Section \ref{sec:orga3cd90f}).  Here, we briefly reviewed the textbook treatment
...
mass case in Section \ref{sec:org16632a3}.  While previous studies typically
...
\label{sec:org69c67a2}
\subsection{Scaling the Hamiltonian}
\label{sec:orgc30777c}
...
in Appendix \ref{sec:org69c67a2}, which is generated by the infinitessimal rotation

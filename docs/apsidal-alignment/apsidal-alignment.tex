...
used in the literature.  In Section \ref{sec:orge9e4d32}, we present the classical picture of resonant capture and
...
conditions which robustly lead to apsidal alignment.  In Section \ref{sec:org83929ab},
...
\ref{sec:org0ecffe1}, where we design a toy model which relies on
...
Section \ref{sec:orge9e4d32} for the test particle.
...
\(q\to\infty\)), see Section \ref{sec:org054db4a}. In the same section,
...
found in Section \ref{sec:orge9e4d32}.
...
outer planet.  As we saw earlier in Section \ref{sec:org83929ab}, the
...
Section \ref{sec:org83929ab}, with the goal of a heuristic
...
which we numerically investigated in Section \ref{sec:org0ecffe1} (i.e. the
...
quantities, we use \(e_{\rm eq}\) (\(= h\sqrt{1.73/j}\), see equation
...
\label{sec:org2c94dc2}
We again adopt the notation used in Section \ref{sec:org83929ab} for \(q=\infty\).
...
the aligned and circulating cases (not plotted), just as \(\hat\theta\)
librates in the \(q=2\) comparable mass case (c.f. Figures
\ref{fig:standardex} and \ref{fig:drivingex}).
Hence, the upper left triangular regions (apsidal circulation) of phase space for \(q=\infty\)
has both \(\theta\) and \(\hat\theta\) (\(\theta_2\) and \(\hat\theta\)),
while the lower right triangular regions (apsidal alignment) have only
\(\hat\theta\) librating.

The angle \(\theta\) (\(\theta_2\)) therefore determines the behavior of
...
The dashed line in \ldots{} is the line

hence, our analysis seems to \ldots{}

\section{Conclusion}
\label{sec:org6d19511}
...
in Section \ref{sec:orge9e4d32}.  However, there are
...
in the vicinity of an MMR with a planet of mass \(m_p\) (Section \ref{sec:org83929ab}).  Here, we briefly reviewed the textbook treatment
...
mass case in Section \ref{sec:org0ecffe1}.  While previous studies typically
...
\label{sec:org5219249}
\subsection{Scaling the Hamiltonian}
\label{sec:orgdb7c184}
...
in Appendix \ref{sec:org5219249}, which is generated by the infinitessimal rotation

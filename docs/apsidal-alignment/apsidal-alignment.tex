...
used in the literature.  In Section \ref{sec:org9859953}, we present the classical picture of resonant capture and
...
conditions which robustly lead to apsidal alignment.  In Section \ref{sec:org2dfc5ce}, we explore apsidal alignment for a test particle in
the vicinity of an eccentric planet's MMR. These results guide our
analysis in Section \ref{sec:orga08f907}, where we design a toy model which
...
Section \ref{sec:org9859953} for the test particle.
...
\(q\to\infty\)), see Section \ref{sec:org4c8624b}. In the same section,
...
the details of these transformations, see Appendix \ref{sec:org8ddcef4}.
...
found in Section \ref{sec:org9859953}.
...
the line \(e_1=e_2\). This is because the angle \(\hat\theta\) does not
depend on the mass ratio \(q\) (e.g., equation \eqref{hattheta}).  The
perpendicular runs fall slightly above the line \(e_2/e_1=q\), while the
anti-aligned runs fall just under it.
...
outer planet.  As we saw earlier in Section \ref{sec:org2dfc5ce}, the
...
The results of Section \ref{sec:orga707c4c} can be applied to the
...
Section \ref{sec:org2dfc5ce}.  As a first order approximation, we
...
in \ref{fig:tp-grid-ext}.  To calculate the $x$-intercept, we
...
in Section \ref{sec:org9859953}.  However, there are
...
in the vicinity of an MMR with a planet of mass \(m_p\) (Section \ref{sec:org2dfc5ce}).  Here, we briefly reviewed the textbook treatment
...
mass case in Section \ref{sec:orga08f907}.  While previous studies typically
...
effects of eccentricity driving in mean motion resonant systems. Our results
...
Nevertheless, the apsidal alignment in K2-19b and c, along with their
moderate eccentricities and the fact that we observe \(\hat\theta\) to
librate \citep{petit_resonance_2020}, therefore suggests that the
planets could have interacted with an eccentricity driving force in
the past.  However, our second conclusion implies that such a force
threatens to destabilize and destroy the resonance in the future.
...
\label{sec:org8ddcef4}
\subsection{Scaling the Hamiltonian}
\label{sec:org26790f3}

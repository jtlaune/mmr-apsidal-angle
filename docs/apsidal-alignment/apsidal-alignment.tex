...
used in the literature.  In Section \ref{sec:org032a2cb}, we present the classical picture of resonant capture and
...
conditions which robustly lead to apsidal alignment.  In Section \ref{sec:org9e62c2e},
...
\ref{sec:org63e229b}, where we design a toy model which relies on
...
Section \ref{sec:org032a2cb} for the test particle.
...
\(q\to\infty\)), see Section \hyperref[sec:org1f371e4]{Relation to the test particle limit}. In the
...
found in Section \ref{sec:org032a2cb}.
...
outer planet.  As we saw earlier in Section \ref{sec:org9e62c2e}, the
...
Section \ref{sec:org9e62c2e}, with the goal of a heuristic
...
We then compare it to the \(q=2>1\) case, which we numerically investigated in
Section \ref{sec:org63e229b} (i.e., the first two rows Figure \ref{fig:Rhat-grid}).
For consistency, we again adopt the notation used in Section \ref{sec:org9e62c2e}.  The inner planet has mass \(m_p\) (subscript \(p\)) and
...
in Section \ref{sec:org032a2cb}.  However, there are
...
in the vicinity of an MMR with a planet of mass \(m_p\) (Section \ref{sec:org9e62c2e}).  Here, we briefly reviewed the textbook treatment
...
mass case in Section \ref{sec:org63e229b}.  While previous studies typically
...
\label{sec:org2bc1dfc}
\subsection{Scaling the Hamiltonian}
\label{sec:org05e1d03}
...
in Appendix \ref{sec:org2bc1dfc}, which is generated by the infinitessimal rotation

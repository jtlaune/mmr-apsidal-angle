...
used in the literature.  In Section \ref{sec:orgdbaf256}, we present the classical picture of resonant capture and
...
conditions which robustly lead to apsidal alignment.  In Section \ref{sec:org4502da9}, we explore apsidal alignment for a test particle in
the vicinity of an eccentric planet's MMR. These results guide our
analysis in Section \ref{sec:orgec33950}, where we design a toy model which
...
Section \ref{sec:orgdbaf256} for the test particle.
...
\(q\to\infty\)), see Section \ref{sec:org12eca75}. In the same section,
...
found in Section \ref{sec:orgdbaf256}.
...
outer planet.  As we saw earlier in Section \ref{sec:org4502da9}, the
...
\label{sec:org12eca75}
The results of Section \ref{sec:org3ed9859} can be applied to the
...
Section \ref{sec:org4502da9}.  As a first order approximation, we
...
in Section \ref{sec:orgdbaf256}.  However, there are
...
in the vicinity of an MMR with a planet of mass \(m_p\) (Section \ref{sec:org4502da9}).  Here, we briefly reviewed the textbook treatment
...
mass case in Section \ref{sec:orgec33950}.  While previous studies typically
...
\label{sec:orgf1d9038}
\subsection{Scaling the Hamiltonian}
\label{sec:orgf22b732}
...
in Appendix \ref{sec:orgf1d9038}, which is generated by the infinitessimal rotation

...
\title[Apsidal Angle in MMR]{Apsidal Alignment and Anti-Alignment of Planets in MMR: Disk-Driven Migration and Eccentricity Driving and Damping}
\author[Laune et al.]{
JT Laune,$^{1}$
Laetitia Rodet,$^{1}$
and Dong Lai$^{1}$
...
\date{}
\title{Apsidal Alignment and Anti-Alignment of Planets in MMR: Disk-Driven Migration and Eccentricity Driving and Damping}
\hypersetup{
 pdfauthor={JT Laune},
 pdftitle={Apsidal Alignment and Anti-Alignment of Planets in MMR: Disk-Driven Migration and Eccentricity Driving and Damping},
 pdfkeywords={},
 pdfsubject={},
...
used in the literature.  In Section \ref{sec:org9ab2fbd}, we present the classical picture of resonant capture and
...
conditions which robustly lead to apsidal alignment.  In Section \ref{sec:org2e908e8},
...
\ref{sec:org22c7b9c}, where we design a toy model which relies on
...
Section \ref{sec:org9ab2fbd} for the test particle.
...
\(q\to\infty\)), see Section \ref{sec:org6a9a081}. In the same section,
...
found in Section \ref{sec:org9ab2fbd}.
...
outer planet.  As we saw earlier in Section \ref{sec:org2e908e8}, the
...
Section \ref{sec:org2e908e8}, with the goal of a heuristic
...
which we numerically investigated in Section \ref{sec:org22c7b9c} (i.e. the
...
\eqref{eq:eeqext}) as a proxy for the ``disk-driven eccentricity'' of
the test particle.  One can see in Figure \ref{fig:Rhat-grid} that, for
...
\ref{fig:tp-grid-ext}. In the following, we treat \(\varpi-\text{circulating}\) and
\(\varpi-\text{anti-aligned}\) as being equivalent dynamical states.

\subsubsection{Calculation of the phase space interface}
\label{sec:org65380b1}
We again adopt the notation used in Section \ref{sec:org2e908e8} for \(q=\infty\).
...
\noindent which is the test particle limit of equation \eqref{hattheta},
and we have assumed \(\varpi_p=0\) is constant.  In both the
\(\varpi-\text{aligned}\) and circulating cases, \(\theta_p\) circulates
and \(\hat\theta\) (as written in equation \eqref{eq:tphhat}) librates
around \(0^\circ\). However, \(\theta\) librates around \(180^\circ\) when
\(\varpi\) circulates, and it circulates when \(\varpi\) librates around
\(0\).  This can be understood with the test particle Hamiltonian
(equation \eqref{eq:tpint}): \(\dot\varpi\) is proportional to
\(\cos\theta\), and so \(\varpi\) cannot librate if \(\theta\) remains close
to \(180^\circ\).

The angle \(\theta\) therefore determines the behavior
of \(\Delta\varpi\) in resonance for a test particle.
Consider equation \eqref{eq:tphhat}: since \(\hat\theta\) always librates
around \(0\), then \(\tan\hat\theta\) remains finite.  This implies that
the denominator should not reach \(0\).  Because \(\theta_p\) always
circulates, \(\cos\theta_p\) takes on values between \(-1\) and \(1\).
Thus, when \(\theta\) librates around \(180^\circ\), the following
constraint is required for \(\hat\theta\) to librate.
\begin{align}
  \label{eq:tphhat}
...
\noindent
Assuming \(e=e_{\rm eq} + \Delta_e\), we suppose that \(\Delta_e\) is constant
within our parameter range and use the data in Figure \ref{fig:tp-grid-ext} to estimate its value
for \(j=2\),
\begin{equation}
\Delta_e \sim 0.015.
\end{equation}
...
\noindent
Finally, using equation \eqref{eq:eeqext} to relate \(h\) and \(e_{\rm eq}\), we estimate
the transition to aligned perihelia to occur for \(h<h_c(e_p)\), where \(h_c\)
is given by
\begin{equation}
\label{eq:phasespaceline}
h_c(e_p) = \sqrt{\frac{j}{1.73}}\left(\left|\frac{f_1}{f_2}\right|e_p-0.015\right)
\end{equation}
...
comparable mass context is to identify the disk driven eccentricities \(e_{1,d}\) and \(e_{2,d}\)
of Section \ref{sec:org22c7b9c} with \(e_{\rm eq}\) and \(e_p\) in Section \ref{sec:org2e908e8}.
...
\(T_e\), and \(e_{i,d}\) would reveal
transitions between the resonance modes analogous to
equation \eqref{eq:phasespaceline} for the comparable mass regime,
but that is beyond the scope of this paper.

\section{Conclusion}
\label{sec:org6f42385}
...
in Section \ref{sec:org9ab2fbd}.  However, there are
...
in the vicinity of an MMR with a planet of mass \(m_p\) (Section \ref{sec:org2e908e8}).  Here, we briefly reviewed the textbook treatment
...
mass case in Section \ref{sec:org22c7b9c}.  While previous studies typically
...
\label{sec:org8ae2edf}
\subsection{Scaling the Hamiltonian}
\label{sec:org3023068}
...
in Appendix \ref{sec:org8ae2edf}, which is generated by the infinitessimal rotation

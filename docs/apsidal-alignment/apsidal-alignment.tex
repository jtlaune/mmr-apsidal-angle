...
used in the literature.  In Section \ref{sec:org95be7d8}, we present the classical picture of resonant capture and
...
conditions which robustly lead to apsidal alignment.  In Section \ref{sec:orgf1e978b},
...
\ref{sec:orgeccca27}, where we design a toy model which relies on
...
Section \ref{sec:org95be7d8} for the test particle.
...
\(q\to\infty\)), see Section \ref{sec:orge2abbe0}. In the same section,
...
found in Section \ref{sec:org95be7d8}.
...
outer planet.  As we saw earlier in Section \ref{sec:orgf1e978b}, the
...
Section \ref{sec:orgf1e978b}, with the goal of a heuristic
...
which we numerically investigated in Section \ref{sec:orgeccca27} (i.e. the
...
\label{sec:org1955d72}
We again adopt the notation used in Section \ref{sec:orgf1e978b} for \(q=\infty\).
...
\(\theta\) librates while \(\theta_p\) circulates (Figure
\ref{fig:tp-circ}).  In the aligned case, neither \(\theta_p\) nor
\(\theta\) librate (Figure \ref{fig:tp-align}).  The angle in equation
\eqref{eq:tphhat} \emph{does} librate in both the aligned and circulating
cases (not plotted), just as in the \(q=2\) comparable mass case
(Figures \ref{fig:standardex} and \ref{fig:drivingex}).

Hence, the upper left triangular regions of phase space for \(q=2\)
(equivalently \(q=\infty\)) has both \(\theta\) and \(\hat\theta\)
(\(\theta_2\) and \(\theta\hat\)).

On the other hand, the lower right
triangular regions (corresponding to apsidal alignment) have only
\(\hat\theta\) librating.  The angle \(\theta\) (\(\theta_2\)) therefore
determines the behavior of \(\Delta\varpi\).

\section{Conclusion}
\label{sec:org7ae033f}
...
in Section \ref{sec:org95be7d8}.  However, there are
...
in the vicinity of an MMR with a planet of mass \(m_p\) (Section \ref{sec:orgf1e978b}).  Here, we briefly reviewed the textbook treatment
...
mass case in Section \ref{sec:orgeccca27}.  While previous studies typically
...
\label{sec:org65b4c21}
\subsection{Scaling the Hamiltonian}
\label{sec:org5e5f9e0}
...
in Appendix \ref{sec:org65b4c21}, which is generated by the infinitessimal rotation

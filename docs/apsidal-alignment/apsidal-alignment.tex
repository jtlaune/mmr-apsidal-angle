...
used in the literature.  In Section \ref{sec:org048cb14}, we present the classical picture of resonant capture and
...
conditions which robustly lead to apsidal alignment.  In Section \ref{sec:org3f10c3c},
...
\ref{sec:orgbb190c0}, where we design a toy model which relies on
...
Section \ref{sec:org048cb14} for the test particle.
...
\(q\to\infty\)), see Section \hyperref[sec:org1863f4b]{Relation to the test particle limit}. In the
...
found in Section \ref{sec:org048cb14}.
...
outer planet.  As we saw earlier in Section \ref{sec:org3f10c3c}, the
...
Section \ref{sec:org3f10c3c}, with the goal of a heuristic
...
which we numerically investigated in Section \ref{sec:orgbb190c0} (i.e. the
...
\ref{fig:tp-grid-ext} (\(h\)) \emph{not} equivalent to the vertical axis in
...
equivalent dynamical states.  We therefore conclude that the
\(\varpi-\text{circulating}\) and \(\varpi-\text{anti-aligned}\) states
correspond with each other in the limit \(q\to\infty\).  Similarly, the
apsidally-aligned regions of the \(q=\infty\) and \(q=2\) parameter spaces
also correspond with each other.

For consistency, we again adopt the notation used in Section \ref{sec:org3f10c3c} for \(q=\infty\).
...
Figures \ref{fig:tp-ep0} - \ref{fig:tp-align} depict the capture outcome
of an external test particle for planets with mass ratios
\(\mu_p=10^{-4}\) and eccentricities \(e_p=0\), \(0.04\), and \(0.054\)
embedded within
a disk with aspect ratio \(h=0.03\).
The aspect ratio sets the equilibrium value of \(e=e_{\rm eq}\)
for \(e_p=0\) (equation \eqref{eq:eeqext}), which corresponds
to Figure \ref{fig:tp-ep0}.

For \(e_p>0\)
the test particle reaches 

Whether or not the test particle becomes aligned with the planet can
be understood as a competition between the value of \(e\) and \(e_p\) in resonance.
For a test particle in resonance with \(e_p>0\), we observe \(e\) to take
on a range of values between 0 and \(\sim1.6e_{\rm eq}\) (Figures
\ref{fig:tp-circ} - \ref{fig:tp-align}).
...
in Section \ref{sec:org048cb14}.  However, there are
...
in the vicinity of an MMR with a planet of mass \(m_p\) (Section \ref{sec:org3f10c3c}).  Here, we briefly reviewed the textbook treatment
...
mass case in Section \ref{sec:orgbb190c0}.  While previous studies typically
...
\label{sec:org36b64b7}
\subsection{Scaling the Hamiltonian}
\label{sec:org5eab672}
...
in Appendix \ref{sec:org36b64b7}, which is generated by the infinitessimal rotation

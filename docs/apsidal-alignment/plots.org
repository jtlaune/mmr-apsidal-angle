...
[[file:./.ob-jupyter/78d7fcf3b94bda9fed42392ebbb18872de41145b.png]]
:END:
** perp example
#+BEGIN_SRC jupyter-python :session /jpy:localhost#8888:research
  os.chdir("/home/jtlaune/Dropbox/multi-planet-architecture/runs/Rhat-grid/")
  ...
  os.chdir("/home/jtlaune/Dropbox/multi-planet-architecture/docs/apsidal-alignment/")
  filename = f"driving-perpendicular-example-h-{h}-Tw0-{int(Tw0)}.png"
...
: 16
: e1d-0.0-e2d-0.2.npz
: driving-perpendicular-example-h-0.03-Tw0-1000.png
[[file:./.ob-jupyter/ad0d5b00dc3816b9b8b7c33a6903e020dee4352a.png]]
:END:
** grid
*** gen behaviortxt
#+BEGIN_SRC jupyter-python :session /jpy:localhost#8888:research
      ...
      for cut_iter in cuts:
          ...
          if cut_iter == cuts[0]:
              ax.scatter(e1*cos(pi),e1*sin(pi),
                         c="k",label=r"$\propto\mathbf{e_1}$",zorder=2)
              ax.scatter(e2*cos(pi-Dpoms),e2*sin(pi-Dpoms),
                         c="r",label=r"$\propto\mathbf{e_2}$",zorder=1)
              ax.scatter(ehat*cos(pi-(g1-hatgamma1)),ehat*sin(pi-(g1-hatgamma1)),
                         c="g",label=r"$\propto\mathbf{\hat e}$",zorder=0)
          else:
              ax.scatter(e1*cos(pi),e1*sin(pi),
                         c="k",zorder=2)
              ax.scatter(e2*cos(pi-Dpoms),e2*sin(pi-Dpoms),
...
#+BEGIN_SRC jupyter-python :session /jpy:localhost#8888:research
  ...
  for axi in ax.flatten():
      axi.set_xlim(-6,4)
      axi.set_ylim(-4,6)
      axi.set_ylabel(r"$\propto\sqrt R\sin\gamma$")
      axi.set_xlabel(r"$\propto\sqrt R\cos\gamma$")
  fig.subplots_adjust(wspace=0.3,hspace=0.4)
  ax[0,0].text(0.5,4,r"$e_{1d}=0$"+"\n"+r"$e_{2d}=0$")
  ax[1,0].text(0.5,4,r"$e_{1d}=0.1$"+"\n"+ r"$e_{2d}=0$")
  ax[2,0].text(0.5,4,r"$e_{1d}=0$"+"\n"+ r"$e_{2d}=0.1$")
  
  ax[0,0].text(-5,-3,r"$t=0-127$ orbits",c="r")
  ax[0,1].text(-5,-3,r"$t=34-38,000$ orbits",c="r")
...
#+begin_example
  ...
  33941.2386344164 38183.89346371845
#+end_example
[[file:./.ob-jupyter/339f74a9050960f7e6b916602826f06ea9944d0b.png]]
:END:
*** in e hat ref frame
#+BEGIN_SRC jupyter-python :session /jpy:localhost#8888:research
  def plotorbit(plot_iter, cuts, fig, ax):
...
#+begin_example
  ...
  33941.2386344164 38183.89346371845
#+end_example
[[file:./.ob-jupyter/0388510eaf416678549044beec3a9f0207ad3dcf.png]]
:END:
* apsidal alignment explanation
** S2 combined plot
#+BEGIN_SRC jupyter-python :session /jpy:localhost#8888:research
  def plot_sS1S2(iplot, ax1, ax2):
      os.chdir("/home/jtlaune/Dropbox/multi-planet-architecture/runs/Rhat-grid/")
      #################
      # CONFIGURATION #
      #################
...
#+BEGIN_SRC jupyter-python :session /jpy:localhost#8888:research
  ...
  plot_sS1S2(0, ax[0,0], ax[0,1])
  plot_sS1S2(1, ax[1,0], ax[1,1])
  plot_sS1S2(8, ax[2,0], ax[2,1])
...
: 16
: e1d-0.0-e2d-0.2.npz
[[file:./.ob-jupyter/d96935beedea18f86f1f2638ac114ba147f98618.png]]
:END:

...
#+end_src
@@latex:\noindent@@
Complete separation of the $\theta$ and $\hat\theta$ resonances occurs whenever
the quantity
$(S_{\notin\theta}-S_{\in\theta})/S_{\in\theta}\gg 1$.
Hence, the $(e_p,e)\text{-phase}$ space should then be split between apsidal alignment and circulation
...
Section [[Test Particle Results]].  As a first order approximation, we
...
in Figure ref:fig:tp-grid-ext.  To calculate the @@latex:$x$-intercept@@, we
...
[[citep:moutamid14_coupl_between_corot_lindb_reson][e.g.][]]
*** Reducing rotation

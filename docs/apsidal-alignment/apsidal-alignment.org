...
$q\to\infty$), see Section [[*Relation to the test particle limit][Relation to the test particle limit]]. In the
...
for the boundary between the @@latex:$\varpi$-aligned and
...
** Relation to the test particle limit
*** notes                                                        :noexport:
...
a canonical shift, equivalent to the reducing rotation of
...
in Figure ref:fig:tp-grid-ext), which is the formal
...
We now compare the $q=\infty$ case to the
$q=2$ case, which we numerically investigated in Section [[*Apsidal
Angle]] (i.e. the first two rows Figure ref:fig:Rhat-grid).
In this configuration, traversing Figure ref:fig:tp-grid-ext along the
horizontal $e_p$ axis is equivalent to traversing the horizontal
$e_{1,\rm d}$ axis in Figure ref:fig:Rhat-grid.  One can see in Figure
ref:fig:Rhat-grid that, for $q=2$ systems that do not escape, the
division between $\varpi-\text{aligned}$ and
$\varpi-\text{anti-aligned}$ roughly corresponds with the division
between $\varpi-\text{aligned}$ and $\varpi-\text{circulating}$ for
the test particles in Figure ref:fig:tp-grid-ext.
We therefore argue that the $\varpi-\text{circulating}$
and $\varpi-\text{anti-aligned}$ states correspond with
each other in the limit $q\to\infty$.

Because the outer planet's behavior in the $q>1$ comparable mass
regime approaches the $q=\infty$ test particle dynamics in the limit
$q\to\infty$, there must be a critical value of $q$ (or range of $q$
values) where the correspondence is not so clear.  However, the
rigorous details of this regime are not clear and beyond the scope of
this paper. For our purposes, it suffices to treat 
$\varpi-\text{circulating}$ and $\varpi-\text{anti-aligned}$ as
being equivalent dynamical states.


For consistency, we again adopt the notation used in Section [[*Test
Particle Results]].  The inner planet has mass $m_p$ (subscript $p$) and
the outer planet is a test particle (no subscript). 
We denote the angles which correspond to $\theta_1$ and $\theta_2$ as
$\theta_p=(j+1)\lambda-\lambda_p$ and $\theta= \theta_p-\varpi$.

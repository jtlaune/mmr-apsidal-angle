    ...
    for eeq0 in eqs:
        ...
        for ep in eps:
            for om_eff in om_effs:
                ...
                dirname = "testsuite/{}/{:0.2e}/eq{:0.2e}/ep{:0.2e}/".format(
                    loc, mup, eeq0, ep
                )
                ...
                collect = (
                    "testsuite/collect/"
                    + case
                    + "-eq{:0.2e}-ep{:0.2e}-om{:0.2e}.png".format(eeq0, ep, om_eff)
                )
                ...
                if os.path.exists(filename):
                    ...
                    print(
                        "{}: running eeq{:0.2e} ep{:0.2e} omeff{:0.2e}...".format(
                            i, eeq0, ep, om_eff
                        )
                    )
                    ...
                    (
                        ...
                        x,
                        y,
                    ) = sim.int_Hsec(0, T, 1e-6, Tm, Te, om_eff, aext)
                    if not os.path.isdir(dirname):
                        os.makedirs(dirname, exist_ok=True)

                    np.savez(
                        filename, teval=teval, thetap=thetap, L=L, g=g, G=G, x=x, y=y
                    )
                    ...
                    suptitle = (
                        ...
                        "{:0.2e}$".format(mup, ep, eeq0, om_eff)
                    )
                    tscale = 1e5

                    print(
                        "{}: plotting eeq{:0.2e} ep{:0.2e} omeff{:0.2e}...".format(
                            i, eeq0, ep, om_eff
                        )
                    )
                    plotsim(

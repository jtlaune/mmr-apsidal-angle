...
class resonance:
    # Parent class for resonance with useful functions.  Mostly just
    # points to the appropriate helper.py functions. Includes the main
    # integrator.
    def __init__(self, j):
    ...
    def H2dof(
        ...
        # including because we need to examine small changes in the
        # resonant location
        ...
        if migrate:
            # Here we're using time = tau*t Add in the dissipative
            # terms for both migration and eccentricity damping
            Tm = self.Tm * self.tau
            Te = Te * self.tau
            ...
            # Here we're using time = tau*t. Add in the dissipative
            # terms for only eccentricity damping. i.e., Tm = infty.
            Te = Te * self.tau
            Ldot = Ldot + (L / 2) * (-4 * G / L / Te)
            xdot = xdot + cos(g) * sqrt(G) * (-1.0 / Te + 0.25 * (-4 * G / Te / L))
            ydot = ydot + sin(g) * sqrt(G) * (-1.0 / Te + 0.25 * (-4 * G / Te / L))

        # lambda_diff is the first 2 terms of the resonance angle.
        if loc == "internal":
            lambda_diff = (j + 1) * n_p / self.tau - j * ldot
        elif loc == "external":
            lambda_diff = (j + 1) * ldot - j * n_p / self.tau

        return np.array([lambda_diff, Ldot, xdot, ydot])


class tp_intH(resonance):
    # This class integrates the Hamiltonian for a test particle near a
    # j:j+1 MMR (internal or external).
    ...
    def tp_H2dof(self, t, Y):
        ...
        lambda_diff, Ldot, xdot, ydot = self.H2dof(
            t,
            theta,
            L,
            x,
    ...
    def int_Hsec(self, t0, t1, tol, Tm=None, Te=None, om_eff=None, aext=None):
        ...
        # scaled H by GM/ap => tau -> sqrt(GM/ap) t;
        # Lambda -> Lambda/sqrt(GM/ap)

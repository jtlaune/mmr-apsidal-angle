...
class test_inwards_simple:
    ...
    def __init__(self, Tm, Te, omext=None):
        self.Tm = Tm
        self.Te = Te
        self.omext = omext
    def force(self, reb_sim):
        ...
        if self.omext:
            preom = (self.omext*(16*np.pi*np.pi*np.pi*tpart.a*tpart.a)
                     /(tpart.n*r2*r2*np.sqrt(r2)))
            # "emperical" corrections
            preom = -preom/(4*np.pi) # not sure where extra factor of
                                    # -4pi comes from, but check
                                    # test-omext.py. figured this out
                                    # emperically
            tpart.ax += -tpart.x*preom 
            tpart.ay += -tpart.y*preom

...
# Varying parameters #
######################
E1_0 = np.ones(Nqs)*0.1
E2_0 = np.ones(Nqs)*0.001
E1DS = np.ones(Nqs)*0.1
...
# THREADING ARRAYS #
####################
G1_0 = np.ones(Nqs)*0. #np.array([np.random.uniform(0, 2*np.pi) for i in range(Nqs)])
G2_0 = np.ones(Nqs)*0. #np.array([np.random.uniform(0, 2*np.pi) for i in range(Nqs)])
HS = np.ones(Nqs)*h
JS = np.ones(Nqs)*j
A0S = np.ones(Nqs)*a0
QS = qs

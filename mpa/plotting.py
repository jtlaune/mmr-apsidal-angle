...
def loadsim(filename, ap, j, ep):
    ...
    barg = np.arctan2(e1 * np.sin(g), ((e1 * np.cos(g) + B / A * ep)))
    barg = (barg) % (2 * np.pi)
    ebar = np.sqrt(
        e1**2 + 2 * B / A * ep * e1 * np.cos(g) + B**2 / A**2 * ep**2
    )
...
def plotphase(fig, axes, teval, suptitle, tscale, fontsize, *argv):
    ...
    for i, value in enumerate(argv):
        ...
        if i == N - 1:
            ...
            # create color bar
            axColor = plt.axes(
                [box.x0 * 1.05 + box.width * 1.05, box.y0, 0.01, box.height]
            )
            plt.colorbar(
                cs,
                cax=axColor,
                orientation="vertical",
                label="t [{:0.1e} orbits]".format(tscale),
                fontsize=fontsize,
            )
